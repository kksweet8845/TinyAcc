...
def gen_conv_fetched_data(mat, ksize):
    ...
    gmat = np.zeros((row, channel, ksize, col), dtype=int)

    zeros = np.zeros((col), dtype=int)

    p = (ksize - 1) >> 1

    low_interval = [ x for x in range(p)]
    uppper_interval = [x for x in range(row+p, row+2*p)]
    middle_interval = [x for x in range(p, p+row)]

    for row_base in range(row):
        for channel_index in range(channel):
            for row_base_cnt, row_base_index in enumerate(range(row_base, row_base + ksize)):
                if (row_base_index in low_interval) or (row_base_index in uppper_interval):
                    # gmat[channel_index, row_base_cnt] = zeros
                    gmat[row_base, channel_index, row_base_cnt] = zeros;
                elif row_base_index in middle_interval:
                    # z = np.zeros((col), dtype=int)
                    # z = mat[channel_index, row_base_index - p]
                    gmat[row_base, channel_index, row_base_cnt] = mat[channel_index, row_base_index - p]
    
    gmat = np.reshape(gmat, (row, channel*ksize, col))
...
def write_instr_(fd, op, rd, rs1, rs2):

    def gen_rs(t):
        src, channel, row, col = t
...
def main():
    ...
    if args.conv:
        ...
        write_instr(fd, CONFIG_OP, 0, channel, row, col, 0, 1, ksize**2, knum)
    ...
    AG_IR_txtpath = os.path.join(args.output, A_IR_txt)
    ...
    if args.conv:
        ...
        gmat = gen_conv_fetched_data(A_mat, ksize)
        # write_binary(AG_filepath, gmat)
        write_txt(AG_IR_txtpath, gmat)
        gmat = gen_conv_rotated_data(gmat, ksize)
        write_binary(AG_filepath, gmat)
        #* Write weighting data
        concat_B = np.concatenate((B_mat,)*(channel * row), axis=0)
